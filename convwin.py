...
# NVEncC (rigaya) — для Windows
NVENC_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "NVEncC")
NVENC_EXE = os.path.join(NVENC_DIR, "NVEncC64.exe")

# Папка по умолчанию для сохранения (Windows)
DEFAULT_OUTPUT_BASE_WIN = os.path.expanduser("~\\Videos\\Rip")
...
class EncodingThread(QThread):
    ...
    def __init__(self, task_id, input_file, output_format, logger=None, task_type='', output_base_dir=None):
        ...
        self.output_base_dir = output_base_dir or DEFAULT_OUTPUT_BASE_WIN
    ...
    def run_nvenc(self, cmd, log_file, output_file):
        """Запуск NVEncC с отслеживанием прогресса (вывод в stderr, парсим процент)."""
        try:
...
class VideoEncoderGUI(QMainWindow):
    ...
    def __init__(self, kafka_available=False):
        ...
        self.progress_bars = {}
        # Папка сохранения (выбирается при первом запуске кодирования; по умолчанию %USERPROFILE%\Videos\Rip)
    ...
    def get_output_base_dir(self):
        """Базовая папка для сохранения (выбранная пользователем или по умолчанию для Windows)."""
        return self.output_base_dir if self.output_base_dir else DEFAULT_OUTPUT_BASE_WIN
    ...
    def start_encoding_thread(self, task_id, format_name):
        ...
        try:
            ...
            # 11) При первом запуске кодирования — выбор папки сохранения (Windows)
            if self.output_base_dir is None:
                chosen = QFileDialog.getExistingDirectory(
                    self,
                    "Выберите папку для сохранения",
                    DEFAULT_OUTPUT_BASE_WIN,
